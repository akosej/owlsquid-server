...
func ActionRun(userLog string, bytes float64, dateJoin string, urlLog string, ipRemote string) (bool, string) {
	...
	if OwlServerDBType == "redis" {
		...
		var user NavigationUsers
		DB.Where("Email = ?", userLog).Find(&user)
		if user.Bloquear == true {
			_, _ = RunString("tcpkill -i " + OwlInterface + " -9 host " + ipRemote + " and port 3128 > /dev/null 2>&1 &")
			//--Reload Squid
			_, _ = RunString(OwlRunOrder)
			...
			if user.ID == 0 {
				...
				user.Name = userLog
				user.Email = userLog
				user.Quota = bytesQuotaDefault
				user.Used = bytes
				user.Update = dateJoin
				user.Last_url = urlLog
				user.Last_size = bytes
				user.Activa = true
				user.Ilimitada = false
				user.Bloquear = false
				DB.Save(&user)
				fmt.Println("Create: " + userLog)
				return true, "Create " + userLog
			} else {
				quota := user.Quota
				used := user.Used
				accumulated := used + bytes
				if quota > used && quota >= accumulated {
					user.Used = user.Used + bytes
					user.Update = dateJoin
					user.Last_url = urlLog
					user.Last_size = bytes
					user.Activa = true
